...
public final class Desechos extends javax.swing.JFrame {    
    ...
    int cantidad;
    String motivo;
    ArrayList<Desechos> des;
    ProductoDesechado product;
    public Desechos() {               
        initComponents();        
    }
    public void mostrarProductDesecho(){
        des = product.obtenerHistorial();
        DefaultTableModel listDesecho = (DefaultTableModel) tabListaDesecho.getModel();
        int i = 0;
        Object[] lista = new Object[5];        
        while(i < des.size()){
            idMedicamento = des.get(i).getMedicamento();
            lista[0] = idMedicamento;
            fecha = des.get(i).getFecha();
            lista[1] = fecha;
            perdida = des.get(i).getTotalPerdida();
            lista[2] = perdida;
